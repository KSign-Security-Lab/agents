...
async def complete_with_tools(
    ...
    *,
    ...
    """Offer several tools with ``tool_choice="auto"`` and run the call /
    execute / append loop until the model answers in prose or ``max_rounds``
    is hit.

    Distinct from ``complete_json``, which forces exactly one function purely
    for structured output; here the model may call zero, one, or several
    tools per round and sees their results before its next turn. Returns
    ``(final_text, trace)``, ``trace`` being every ``{"tool","args","result"}``
    in call order, for the caller to turn into citable sources.
    """
    msgs = list(messages)
    ...
    for _ in range(max_rounds):
        ...
        for tc in msg.tool_calls:
            ...
            try:
                args = _loads_lenient(tc.function.arguments)
